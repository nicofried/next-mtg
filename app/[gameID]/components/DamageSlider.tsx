...
interface DamageSliderProps {
  ...
  const DamageSlider: FC<DamageSliderProps> = ({ onConfirm }) => {
  ...
  return (
    <div style={{ top: "50%", display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1px', justifyContent: 'center' }}>
      <div style={{ top: "50%",padding:'1px',color: value > 0 ? 'limegreen' : value < 0 ? 'crimson' : '' }}>
        
      </div>
  
      <div style={{top: "50%", display: 'flex', gap: '1px', alignItems: 'center', width: '100%', height:'100%', justifyContent: 'center'}}>
        <Button onClick={decrement} variant="secondary">-</Button>
        
        {/* Explicitly set a minimum height for the Slider container */}
        <div style={{ padding: "1px", top: "50%", flexGrow: 1, minHeight: '2em', transition: 'all' }}> 
          <Slider
